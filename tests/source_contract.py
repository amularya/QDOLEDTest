...
required_source = {
    ...
    "Win32 control helper macros": "#include <windowsx.h>",
}
...
assert "_WIN32_WINNT=0x0A00" in cmake, "Windows 10 APIs must be declared consistently"
...
assert 'VS_GLOBAL_GenerateManifest "false"' in cmake, "Visual Studio must not generate a second manifest"
assert 'VS_GLOBAL_EmbedManifest "false"' in cmake, "Visual Studio must not embed a duplicate manifest"
assert "runs-on: windows-2022" in workflow, "workflow must use the stable Visual Studio 2022 runner"
assert "Verify single-manifest configuration" in workflow, "workflow must reject duplicate manifest generation"
assert "Verify Windows executable" in workflow, "workflow must validate its PE output"
assert "Summarize compiler failure" in workflow, "workflow must put concise compiler errors in the run summary"
